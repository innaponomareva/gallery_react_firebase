...
import { addPhoto_fb, getAllPhotos_fb, removePhoto_fb, updatePhoto_fb } from '../../service/photoService';
...
export const PhotoState = ({children}) => {
  ...
  const removePhoto = async({id, fileNameInStorage}) => {
    try{
      setRemovePhoto(id);
      removePhoto_fb({id, fileNameInStorage});
    }catch(error){
      return Promise.reject(error);
    }
  }

  const updatePhoto = async(update) => {
    try{
      setUpdatePhoto(update);
      updatePhoto_fb(update)
    }catch(error){
      return Promise.reject(error);
    }
  }
  ...
  const setUpdatePhoto = update => dispatch({type: UPDATE_PHOTO, payload: update});
  const setGetAllPhotos = data => dispatch({type: GET_ALL_PHOTOS, payload: data});
  const setLoading = () => dispatch({type: SET_LOADING});
  const stopLoading = () => dispatch({type: STOP_LOADING});
  const setDefineTargetIndex = index => dispatch({type: SET_INDEX, payload: index});
