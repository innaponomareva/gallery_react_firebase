...
export async function onFileChange_fb(event){
  ...
  try{
    ...
    return {
      ...
      size: {width, height, KB: file.size * 0.001, format: calcFormat(width, height)}
    };
  }catch(error){
...
export async function addPhoto_fb({id, created, fileNameInStorage, description, hashtags, fileUrl, size}){
  try{
...
export async function getAllPhotos_fb(){
  try{
    const allPhotos = []; 
    const querySnapshot = await fb.firestore().collection('images').orderBy('created').get();
    if(querySnapshot){
...
export async function cleanStorage_fb(){ // Check junk files in Storage and remove them
  try{
    const allFileNamesInFirestore = await getAllFileNamesInFirestore();
    const { allFileNamesInStorage, storageRef } = await getAllFileNamesInStorage();
